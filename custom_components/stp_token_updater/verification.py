...
from .api import ProviderClient, ProviderConnectionError
from .models import SponsorStatus, TokenUpdateResult, TrialTokenCandidate
...
async def _read_status(client: ProviderClient) -> tuple[SponsorStatus | None, str | None]:
    try:
        return await client.async_get_sponsor_status(), None
    except ProviderConnectionError:
        return None, "readback_unreachable"
...
async def async_apply_and_verify(
    ...
    *,
    ...
    """Apply exactly once, then verify with two delayed readbacks.

    Only a connection/timeout failure during POST is treated as an uncertain
    write. Authentication, rate-limit and definite API errors propagate to the
    coordinator. Readback connection failures never trigger a second POST.
    """
    ...
    first, first_read_error = await _read_status(client)
    matched, first_reason = _matches(first, candidate, previous_expiry, tolerance)
    if matched:
        return TokenUpdateResult(
            True,
            previous_expiry,
            candidate.metadata.expires_at,
            first.expires_at if first else None,
            now_fn(),
        )

    await sleep(second_delay)
    second, second_read_error = await _read_status(client)
    ...
    return TokenUpdateResult(
        ...
        previous_expiry,
        candidate.metadata.expires_at,
        second.expires_at if second else (first.expires_at if first else None),
        now_fn(),
        failure,
    )
