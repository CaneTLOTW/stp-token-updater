"""Pure expiry-based scheduler; no network and no HA dependencies."""
...
RENEWAL_WINDOW = timedelta(hours=48)
...
def calculate_schedule(
    *,
    ...
    checkpoints = [
        (expires_at - renewal_window, "t_minus_48h"),
    ...
    if now >= expires_at:
        if last_attempt_at is None or last_attempt_at < expires_at:
            return ScheduleDecision(True, now, "expired")
        next_attempt = last_attempt_at + POST_EXPIRY_RETRY
        return ScheduleDecision(now >= next_attempt, max(now, next_attempt), "expired")
    crossed = max(i for i, (at, _) in enumerate(checkpoints) if now >= at)
    current_at, stage = checkpoints[crossed]
    if last_attempt_at is None or last_attempt_at < current_at:
