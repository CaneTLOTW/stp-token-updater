"""JWT metadata parsing for the public provider trial token."""
...
class TrialTokenParseError(ValueError):
    """The candidate is not a plausible provider trial JWT."""
...
def parse_trial_token(
    ...
    *,
    ...
    """Decode claims only; the provider remains the authority for signature validation."""
...
def select_latest_trial_candidate(
    ...
    *,
    ...
    if not candidates:
        raise TrialTokenParseError("no valid provider trial token found")
    return candidates[-1]
